...
class Admin(Account):
    '''Admin class for managing the car rental system. Inherits from Account class.'''
    admin_file = Account.get_data_path('data/admins.json') 
    ...
    def verify_admin_key(input_key):
        '''Verifies the admin key from the config file.'''
        try:
            with open(Account.get_data_path('data/config.json'), "r") as f:
                config = json.load(f)
    ...
    @staticmethod
    def load_admin():
        '''Loads the admin data from the JSON file.'''
        try:
    ...
    def save_admin(self):
        '''Saves the admin data to the JSON file.'''
        ...
        st.success(f"Admin {self.first_name.upper()}-{self.last_name.upper()} Account created successfully!")
    ...
    def __sub__(self, car_id):
        '''Removes a car from the system based on the car_id.'''
        cars = Car.load_cars()
        if cars.empty:
